...
const KotakMemori = () => {
  ...
  const startGame = () => {
    ...
    setTimeout(() => {
      ...
      setTimer(newTimer);
    }, 1000); // Jeda sebelum kotak kuning muncul
  };
  ...
  const handleCellClick = (index) => {
    ...
    if (correctCells.includes(index)) {
      const newCorrectCells = correctCells.filter((cell) => cell !== index);
      setCorrectCells(newCorrectCells);
      
      if (newCorrectCells.length === 0 && clickedCells.length === (level + 2 - 1)) {
        setTimeout(() => {
          if (round < 3) {
            setRound(round + 1);
            startGame();
            ...
            if (level < 3) {
              ...
              setMessage("");  // Menghilangkan pesan "Selamat permainan selesai"
            }
          }
        }, 500);
      }
    ...
    <div className={styles.container}>
      ...
      {tutorial ? (
        <div className={styles.tutorial}>
          <p>Perhatikan kotak berwarna biru tua yang muncul.</p>
          <p>Kamu harus mengingat posisinya dimana, lalu pilih kotak yang tadi berwarna biru tua.</p>
          <button onClick={() => {
            setTutorial(false);
        ...
        <>
          ...
          {!gameFinished && (
            ...
            >
              {[...Array(gridSize * gridSize)].map((_, i) => (
                <div
                  key={i}
                  className={`${styles.gridCell} ${showAllBlue ? styles.blue :
                    showYellow && correctCells.includes(i) ? styles.yellow : styles.blue
          ...
          <p className={styles.message}>
            {gameFinished ? (
              <>
                <img src="/assets/afasia/clapping.svg" alt="Tepuk Tangan" className={styles.clappingImage} />
